...
public class EtcdServiceDiscovery {
    ...
    private Map<String, TreeSet<String>> app2ServiceList = new HashMap<>();
    ...
    public synchronized String findService(String applicationName) throws Exception {
        ...
        if(app2ServiceList.get(applicationName)==null){
            final TreeSet<String> serviceProps = new TreeSet<>();
            Properties props = etcdClient.watch(path, event->{
                String value = event.getValue();
                serviceProps.add(value);
            });
            logger.info(props.toString());
            serviceProps.addAll(Arrays.asList(props.values().toArray(new String[0])));
            app2ServiceList.put(applicationName,serviceProps);
        }
        String value = app2ServiceList.get(applicationName).first();
